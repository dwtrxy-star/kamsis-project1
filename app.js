...
function getKey() {
  const keyBytes = textToBytes(document.getElementById("key").value);
  if (keyBytes.length < 8) {
    throw new Error("Kunci minimal 8 karakter.");
  }
  return keyBytes.slice(0, 8);
...
function getIV(mode) {
  ...
  const ivBytes = textToBytes(document.getElementById("iv").value);
  if (ivBytes.length < 8) {
    throw new Error("IV minimal 8 karakter untuk CBC/CFB.");
  }
  return ivBytes.slice(0, 8);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  updateIVState();
>>>>>>> 949afa3d3312ab574b5eab3dd8bb486f7d33f70a
});